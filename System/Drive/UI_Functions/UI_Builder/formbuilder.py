...
class CustomFrame:
    ...
    def __init__(self, bgImage=None):
        ...
        if bgImage is not None:
            ...
            self.img = ImageTk.PhotoImage(self.file.resize((464, 164)))
    ...
    def setupLabel(self, x, y, width, height, text=None):
        if self.bgCheck:
    ...
    def setupIconButton(self, x, y, iconPath, iconScale=1, command=lambda: print("Hi Mom!!")):
        if self.bgCheck:
            ...
            self.buttonWindow = self.canvas.create_window(x, y, anchor=tk.NW)
        else:
